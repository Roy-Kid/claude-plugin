...
| Skill | Purpose |
|---|---|
| `/mol-plugin:new-skill` | Scaffold a new skill in any plugin (`mol`, `mol-plugin`). Authors a complete, runnable SKILL.md (no TODO placeholders) and appends one row to that plugin's `README.md` skills table. Runs `/mol-plugin:check` at the end so the new skill is verified before exit. Does not touch `plugin.json`. |
| `/mol-plugin:check` | The marketplace's self-audit. Structural check across the whole marketplace: `marketplace.json`, every `plugin.json`, every `SKILL.md`, every agent. Parallel to `/mol:bootstrap`, but for the plugin source rather than a project's harness. Read-only. |
